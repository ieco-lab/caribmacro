#' Find the Minimum Value of a Row
#'
...
#'
#' @return A vector of the minimum values of each row of \code{data}. If a row has no values (i.e.
...
row_min <- function(data, na.rm = FALSE) {
  out <- NULL
  for (i in 1:nrow(data)) {
    if (any(!is.na(data[i, ]))) {
      out <- c(out, min(data[i, ], na.rm = na.rm))
    } else {
      out <- c(out, NA)
    }
  }
